...
cre2_string_t * 
cre2_strings_duplicate(cre2_string_t * source, int size)
{
  cre2_string_t * target = new cre2_string_t[size];
  for (int i = 0; i < size; i++) {
    target[i].data = strndup(source[i].data, source[i].length);
    target[i].length = source[i].length;
  }
  return target;
}
void
cre2_strings_free(cre2_string_t *strings, int size)
{
  for(int i = 0; i < size; i++) {
...
int
cre2_match_ex (const cre2_regexp_t *re , const char *text,
	    int textlen, int startpos, int endpos, cre2_anchor_t anchor,
	    cre2_string_t *match, cre2_range_t * ranges, int nmatch)
{
  re2::StringPiece	text_re2(text, textlen);
  std::vector<re2::StringPiece>	match_re2(nmatch);
  RE2::Anchor		anchor_re2 = to_cre2_anchor(anchor);
  ...
  if (retval) {
    for (int i=0; i<nmatch; i++) {
      match[i].data   = match_re2[i].data();
      match[i].length = match_re2[i].length();
      ranges[i].start = match[i].data - text;
      ranges[i].past  = ranges[i].start + match[i].length;
    }
  }
  return (retval)? 1 : 0;
}
